...
const Reserve = ({ setOpen, trainId }) => {
  const [selectedTickets, setSelectedTickets] = useState([]);
  const { data, loading, error } = useFetch(`/trains/ticket/${trainId}`);
    ...
    try {
      await Promise.all(
        selectedTickets.map((ticketId) => {
          const res = axios.put(`/tickets/availability/${ticketId}`, {
            dates: alldates,
          });
          return res.data;
        })
      ...
      <div className="rContainer">
        ...
        {data.map((item) => (
          <div className="rItem" key={item._id}>
            ...
            <div className="rSelectTickets">
              {item.ticketNumbers.map((ticketNumber) => (
                <div className="ticket">
                  <label>{ticketNumber.number}</label>
                  <input
                    type="checkbox"
                    value={ticketNumber._id}
                    onChange={handleSelect}
                    disabled={!isAvailable(ticketNumber)}
                  />
                </div>
              ))}
            </div>
